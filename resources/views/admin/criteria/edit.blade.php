...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-amber-50 to-yellow-50">
            <h2 class="text-lg font-bold text-gray-900">Edit Kriteria: <span class="text-blue-600">{{ $criteria->kode }}</span></h2>
            <p class="text-gray-500 text-sm mt-1">Perbarui data kriteria penilaian</p>
        </div>
        <form method="POST" action="{{ route('admin.criteria.update', $criteria) }}" class="p-6 space-y-5">
            @csrf @method('PUT')
            <div>
            ...
            <div>
                ...
                <select id="tipe" name="tipe"
                    ...
                    <option value="benefit" {{ old('tipe', $criteria->tipe) === 'benefit' ? 'selected' : '' }}>Benefit (↑ Semakin besar semakin baik)</option>
                    <option value="cost" {{ old('tipe', $criteria->tipe) === 'cost' ? 'selected' : '' }}>Cost (↓ Semakin kecil semakin baik)</option>
                </select>
            ...
            <div>
                ...
                <input type="number" id="urutan" name="urutan" value="{{ old('urutan', $criteria->urutan) }}" min="1"
